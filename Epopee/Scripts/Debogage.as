package {
	...
	********************************************************************************/
	...
	public class Debogage extends MovieClip {
		...
		private var _yProchainBtnDroiteTab: uint = _marge;
		private var _yProchainBtnGaucheTab: uint = _marge;
		private var _leFrameJeu: uint;
		private var _FPS_INITIAL = 24;
		
		private var droite: Boolean = true;
		...
		******************************************************************************/
		private function init(e: Event): void {
			...
			if (_miseAJourAuto) {
				addEventListener(Event.ENTER_FRAME, changerTxt);
			}
		...
		******************************************************************************/
		private function nettoyer(e: Event = null): void {
			...
			if (_miseAJourAuto) {
				removeEventListener(Event.ENTER_FRAME, changerTxt);
			}
			addEventListener(Event.ADDED_TO_STAGE, init);
			...
			while (numChildren > 0) {
				removeChildAt(0);
			} //élimination de tous les boutons et champs
			...
			if (_phaseEntreeMDP) {
				...
				if (touche == Keyboard.D) { // «ctrl+shift+D» ...
					...
					if (_phaseEntreeMDP == false && _accesDebogage == false) {
						...
						_modeDebogage = !_modeDebogage;
						log("DEBOGAGE=" + _modeDebogage, 2);
						if (_modeDebogage) {
							_jeu.addChild(this);
						} else {
							_jeu.removeChild(this);
						};
						return true; //puisqu'on retourne true, _jeu ne fera rien de plus
					} //if+else
					...
					switch (touche) {
						case Keyboard.NUMBER_5:
							for each(perso in _jeu.getTPersos()) {
								perso.superForceDebogage();
							};
							break; // «ctrl+shift+5» + DÉBOGAGE ...
						case Keyboard.NUMBER_6:
							for each(perso in _jeu.getTPersos()) {
								perso.mauvietteDebogage();
							};
							break; // «ctrl+shift+6» + DÉBOGAGE ...
						case Keyboard.B:
							_jeu.amorcerCombat();
							break; // «ctrl+shift+b» + DÉBOGAGE déclenche une bataille immédiate
						case Keyboard.S:
					...
					if (_jeu.getEcranDeJeu() is Tableau && _jeu.getEstEnDialogue() == false) {
						switch (touche) {
							...
							case Keyboard.NUMBER_8:
								...
								break; // «ctrl+shift+8» + DÉBOGAGE ajoute une patate
							case Keyboard.P:
								_jeu.montrerCtrlPad(true);
								break; // «ctrl+shift+p» + DÉBOGAGE affiche le ctrlPad
							case Keyboard.T:
								testerIntegration();
								break; // «ctrl+shift+t» + DÉBOGAGE teste tous les tableaux pour générer un rapport
						} //switch
				...
				for each(nomTableau in _jeu.getTTableaux()) { //boucle sur tous les tableaux déclarés
					...
					try {
						...
						for (var i = 0; i < ecranEnCours.numChildren - 1; i++) {
							...
							if (unEnfant is Teleport) {
								tTeleports.push(unEnfant);
							...
							} else if (unEnfant is PNJ) {
								tPNJs.push(unEnfant);
							} else if (unEnfant is Objet) {
								tObjets.push(unEnfant);
							} else if (unEnfant is Obstacle) {
								tObstacles.push(unEnfant);
							}
						} //for
		...
		******************************************************************************/
		private function extraireNoms(tClips: Array): Array {
			var tNoms: Array = [];
			for each(var unClip: MovieClip in tClips) {
				tNoms.push(unClip.name);
			}
			tNoms.sort();
			return tNoms;
		} //extraireNoms
		...
		******************************************************************************/
		private function ajouterBtnTableau(ecranDestination: String): void {
			...
			if (droite) {
				unBtnGo.x = 50 - unBtnGo.width;
				unBtnGo.y = _yProchainBtnGaucheTab;
				droite=false
			} else if (droite == false) {
				unBtnGo.x = -90 - unBtnGo.width;
				unBtnGo.y = _yProchainBtnDroiteTab;
				droite=true;
			}
			addChild(unBtnGo);
			unBtnGo.addEventListener(MouseEvent.CLICK, function (): void {
				_jeu.changerEcranJeu(ecranDestination);
				allerAuPremierTeleport(ecranDestination);
			});
			unBtnGo.width=140;
			_yProchainBtnGaucheTab +=_marge;
			_yProchainBtnDroiteTab +=_marge;
		} //ajouterBtnTableau
					...
					try {
						...
						switch (description) {
							...
							case "soigner les monstres":
								for each(monstre in _jeu.getEcranDeJeu().getTMonstres()) {
									monstre.guerir(50);
								};
								break;
							case "presque tuer les équipiers":
								for each(perso in _jeu.getTPersos()) {
									perso.blesser(perso.getPVAct() - 1);
								};
								break;
							case "presque tuer les monstres":
								for each(monstre in _jeu.getEcranDeJeu().getTMonstres()) {
									monstre.blesser(monstre.getPVAct() - 1);
								};
								break;
							case "vaincre ce combat":
								_jeu.getEcranDeJeu().victoire();
								break;
							case "rétablir mana des équipiers":
								for each(perso in _jeu.getTPersos()) {
									perso.setPMAct(perso.getPMMax())
								};
								break;
							case "presque passer un niveau":
								for each(perso in _jeu.getTPersos()) {
									perso.setXPAct(perso.getXPSuivant() - 1)
								};
								break;
							...
							case "animations normales":
								_jeu.setFacteurTemps(1);
								stage.frameRate = _FPS_INITIAL;
								break;
