package  {
	...
	public class Jeu extends MovieClip {
		...
		private var _tTableauxPacifiques:Array = ["Village", "Foret", "MaisonAmi", "MaisonMaman", "Chateau", "SalleDuRoi", "Caverne", "Marchand1", "Village2", "Foret2", "Marais05", "Lac", "RavinChateau", "InterieurTemple", "Temple", "EntreeVille", "Ville", "Marais04", "Entree", "Taverne"];
		...
		private var _gauche:Boolean=false, _droite:Boolean=false, _haut:Boolean=false, _bas:Boolean=false;
		...
		******************************************************************************/
		public function changerEcranJeu(nomEcran:String, nomTeleport:String=null):void {
			...
			if(nomEcran == null){ nomEcran = _memNomEcran; log("...de retour vers «"+nomEcran+"»", 2); } // si nomEcran est null, il faut retourner au tableau mémorisé
			if(_ecranDeJeu != null){ fermetureTableauEnCours(); }
			else { log("c'est le premier tableau...", 3); }
			...
			if(nomEcran.indexOf("Combat")>=0){ //si le nom de l'écran contient le mot combat:
				...
				if(_tTableauxPacifiques.indexOf(nomEcran)>=0){ _combatPossible = false; } //pas de combat dans les tableaux paisibles...
				else{ _combatPossible = true; } //sinon, il y a des combats possibles dans tous les autres tableaux
				...
				if(_ctrlPadActif){ addChild(_ctrlPad); }
				creerBtnMenuInfo();
				_memNomEcran = nomEcran; log("+++_memNomEcran "+_memNomEcran, 3); // au besoin, cette variable sera utilisée au retour des combats
				addEventListener(Event.ENTER_FRAME, loop); 
			} //if+else if+else
		...
		******************************************************************************/
		public function verifierSiCombat():void {
			if(_combatPossible) {
				_prochainCombat -= 1; 
				if(_prochainCombat%20==0){ log("nb de deplacements avant bataille = " + _prochainCombat, 2); } //affichage de l'info à chaque 20 déplacements
				else if(_prochainCombat%5==0){ log("nb de deplacements avant bataille = " + _prochainCombat, 3); } //affichage à chaque 5 déplacements
			} //if
			if(_prochainCombat <= 0) {amorcerCombat();}
		} //verifierSiCombat
		...
		******************************************************************************/
		public function terminerDialogue():void {
			if( this.contains(_dialogue) ){ removeChild(_dialogue); } //si le dialogue est là, on l'enlève (n'est pas là après combat)
		...
		******************************************************************************/
		public function frappeClavier(e:KeyboardEvent):void {
			...
			switch(touche){
				...
				case Keyboard.COMMAND: case Keyboard.CONTROL: case Keyboard.SHIFT: return; //...arret de la fn
			} //switch
			...
			if( _debogage_mc.frappeClavierDebogage(e) ){ 
				...
				switch (touche) {
					case Keyboard.LEFT :  _gauche=true; break;
					case Keyboard.RIGHT : _droite=true; break;
					case Keyboard.UP :    _haut  =true; break;
					case Keyboard.DOWN :  _bas   =true; break;
					case Keyboard.I : ouvrirMenuInfo(); break; // la touche «i» ouvre le menu des infos 
				} //switch
		...
		******************************************************************************/
		public function arretFrappeClavier(e:KeyboardEvent):void {
			...
			if(_ecranDeJeu is Tableau){
				switch (touche) {
					case Keyboard.LEFT :  _gauche=false; break;
					case Keyboard.RIGHT : _droite=false; break;
					case Keyboard.UP :    _haut  =false; break;
					case Keyboard.DOWN :  _bas   =false; break;
				} //switch
				if(!_gauche && !_droite && !_haut && !_bas){ _ecranDeJeu.arreterJoueur(_spero); } //au besoin, arret du cycle de marche
			} //if
		} //arretFrappeClavier
		...
		******************************************************************************/
		public function montrerCtrlPad(forcer:Boolean=false):void{
			if(forcer){ _ctrlPadActif = !_ctrlPadActif; } //inversion de l'état
			if(_ctrlPadActif){
				_ctrlPad = new CtrlPad();
				_ctrlPad.x=80; _ctrlPad.y = 650;
				if(_ctrlPad.parent==null){ addChild(_ctrlPad); } //on l'ajoute, sauf s'il est déjà là...
			} else {
				if(forcer){ if(_ctrlPad!=null){ removeChild(_ctrlPad); } } //si il y a un _ctrlPad, on l'enlève...
			} //if+else
		} //montrerCtrlPad
		...
		******************************************************************************/
		public function creerBtnMenuInfo():void {
			...
			_btMenuInfo.x=1220; _btMenuInfo.y = 660; 
		...
		******************************************************************************/
		private function loop(e:Event):void{
			...
			if(_ecranDeJeu is Tableau){
				var vX:Number=0, vY:Number=0; //les vecteurs de mouvements sont initialisés
				if(_gauche){ vX-=_distance; }
				if(_droite){ vX+=_distance; }
				if(_haut  ){ vY-=_distance; }
				if(_bas   ){ vY+=_distance; }
				...
				if(vX!=0 || vY!=0){ _ecranDeJeu.deplacerJoueur(_spero, vX, vY); } //le déplacement est demandé seulement si il y a un vecteur pertinant!
			} //if(c'est un Tableau)
		} //loop
		...
		******************************************************************************/
		public function soigner(valeurDeSoin:int=int.MAX_VALUE):void{
			for each (var perso:Perso in _tPersos){ perso.guerir(valeurDeSoin, true); }
		} //soigner
		...
		******************************************************************************/
		public function ajouterPerso(nomInstance):void{
			...
			switch(nomInstance){
				case "pnjNova_mc":   lePerso = _nova;   break;
				case "pnjLucem_mc":  lePerso = _lucem;  break;
				case "pnjFortis_mc": lePerso = _fortis; break;
			} //switch
		...
		******************************************************************************/
		public function jouerMusique(nomMusique:String, redemarrage:Boolean=false):void{
			...
			switch(nomMusique){
				case "CombatChef":      nomMusique="MusiqueCombatFinal"; break;
				case "CombatSousChef1":
				case "Combat":          nomMusique="MusiqueCombat";      break;
				case "Ravin":
				case "Rocaille":        nomMusique="MusiqueCornemuse";   break;
				case "Caverne":         nomMusique="MusiqueClavier";     break;
				case "Foret":
				case "Foret2":          nomMusique="MusiqueTambour";     break;
				case "MaisonChamane":   nomMusique="MusiqueHarpe";       break;
				default:                
					if( redemarrage ){
			...
			if( nomMusique == nomMusiqueEnCours && !redemarrage ){
				...
				if(nomMusiqueEnCours != "null"){ arreterMusique(); log("Arrêtez la  musique svp!", 2);}
				
				if(_musiqueOn){ _pisteAudio = _musique.play(0,int.MAX_VALUE); } //la lecture se fera en boucle, presque à l'infini...
			} //if+else
		} //jouerMusique
		...
		******************************************************************************/
		public function montrerAnimFinale(estVictorieux:Boolean):void{
			...
			if( this.contains(_debogage_mc) ){ removeChild(_debogage_mc); } //changement du 8 octobre 2016: on l'enlève seulement si c'est l'enfant de jeu
		...
		******************************************************************************/
		private function fermetureTableauEnCours():void{
			if(_ecranDeJeu is Tableau){
				...
				if(_ctrlPadActif){ removeChild(_ctrlPad); }
		...
		*******************************     GETTERS     *******************************
		******************************************************************************/
		public function getMemPosPerso():Point{ return _memPosPerso; } 
		public function getEstEnDialogue():Boolean{ return _estEnDialogue; } 
		public function getTxtVersion():String{ return _txtVersion; } //pour débogage, ne pas supprimer cette ligne!
		public function getFortune():Number{ return _fortune; } //pour débogage, ne pas supprimer cette ligne!
		public function getDistance():Number{ return _distance; } //pour débogage, ne pas supprimer cette ligne!
		public function getTObjets():Array{ return _tObjets; } //pour débogage, ne pas supprimer cette ligne!
		public function getTPersos():Array{ return _tPersos; } //pour débogage, ne pas supprimer cette ligne!
		public function getTTableaux():Array{ return _tTousLesTableaux; } //pour débogage, ne pas supprimer cette ligne!
		public function getEcranDeJeu():MovieClip{ return _ecranDeJeu; } //pour débogage, ne pas supprimer cette ligne!
		public function getFacteurTemps():Number{ return _facteurTemps; } //pour débogage, ne pas supprimer cette ligne!
		...
		*******************************     SETTERS     *******************************
		******************************************************************************/
		public function setDistance(nb:Number):void{ _distance=nb; } //pour débogage, ne pas supprimer cette ligne!
